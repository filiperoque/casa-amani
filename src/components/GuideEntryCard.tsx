    ...
    <Reveal>
      <article className="border-t border-brown/10 py-8 first:border-t-0 first:pt-0 last:border-b">
        <div className="mb-2 flex items-baseline gap-3">
          ...
          <span className="text-sm text-brown/40">{entry.location}</span>
        </div>
        {entry.tagline && (
          <p className="mb-3 text-sm italic text-brown/50">{entry.tagline}</p>
        )}
        <p className="leading-7 text-brown/70">{entry.body}</p>
        {entry.body2 && (
          <p className="mt-2 leading-7 text-brown/70">{entry.body2}</p>
        )}
        <p className="mt-4 text-sm italic leading-6 text-brown/50">
          {entry.practical}
          {entry.lastVerified && (
